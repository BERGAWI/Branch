odoo.define('branch_accounting_report.account_reporwt', function (require) {
'use strict';
...
var accountReportsWidget = AbstractAction.extend({
    ...
    custom_events: {
         'branch_filter_changed': function (ev) {
            console.log('-------44444444444-----------> ev ', ev);
            var self = this;
            self.report_options.branch_ids = ev.data.branch_ids;
            console.log('-------eeeeeeeee-----------> ev.data.branch_ids ', ev.data.branch_ids);
            console.log('------------------> self.report_options.branch_ids ', self.report_options.branch_ids);
    ...
    render_searchview_buttons: function() {
            ...
            if (this.report_options.branch) {
                if (!this.branch_m2m_filter) {
                    ...
                    if (!_.isEmpty(fields)) {
                        this.branch_m2m_filter = new M2MFilters(this, fields, 'branch_filter_changed');
