    ...
    def _get_reconciled_moves(self, report, options, currency_table_query, payment_account_ids, payment_move_ids,
        ...
        for column_group_key, column_group_options in report._split_options_per_column_group(options).items():
            ...
            params += [
                ...
                column_group_key,  # Added to match the second %s in the second part of the UNION ALL
                column_group_payment_move_ids,
                payment_account_ids,
                column_group_payment_move_ids,
                column_group_options['date']['date_from'],
        ...
        # Verify the number of placeholders and parameters match before executing the query
        if len(queries) > 0 and len(queries[0].split('%s')) - 1 == len(params):
            self._cr.execute(' UNION ALL '.join(queries), params)
        else:
            # Handle the error or log a message
            print("Error: Number of placeholders and parameters do not match.")
        ...
        for aml_data in self._cr.dictfetchall():
            ...
            reconciled_percentage_per_move[aml_data['column_group_key']][aml_data['move_id']].setdefault(
                aml_data['account_id'], [0.0, 0.0])
            reconciled_percentage_per_move[aml_data['column_group_key']][aml_data['move_id']][aml_data['account_id']][
                0] += aml_data['balance']
        ...
        for column in options['columns']:
            ...
            params += [column['column_group_key'],
                       tuple(reconciled_percentage_per_move[column['column_group_key']].keys()) or (None,),
        ...
        for aml_data in self._cr.dictfetchall():
            if aml_data['account_id'] in reconciled_percentage_per_move[aml_data['column_group_key']][
                aml_data['move_id']]:
                reconciled_percentage_per_move[aml_data['column_group_key']][aml_data['move_id']][
        ...
        for column in options['columns']:
            ...
            params += [column['column_group_key'], tuple(cash_flow_tag_ids),
                       tuple(reconciled_percentage_per_move[column['column_group_key']].keys()) or (None,)]
        ...
        for aml_data in self._cr.dictfetchall():
            ...
            elif not total_amount and aml_account_id in reconciled_percentage_per_move[aml_column_group_key][
                aml_move_id]:
